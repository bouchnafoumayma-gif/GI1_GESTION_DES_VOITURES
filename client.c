...
void inscriptionClient() {
    ...
    centrerTexte("Prenom : "); scanf("%s", c.prenom);
    centrerTexte("Email : "); scanf("%s", c.email);
    centrerTexte("Telephone : "); scanf("%s", c.telephone);
    ...
    effacerConsole();
    afficherLogo();
    afficherCadreCentre(GREEN "Inscription reussie !" RESET);
    _getch();
}
...
int connexionClient() {
    ...
    if (!f) {
        centrerTexte(YELLOW "? Aucun client enregistre pour le moment." RESET);
        _getch();
        return -1;
    }
    ...
    while (fscanf(f, "%d %s %s %s %s %s %s\n",
        ...
        if (strcmp(email, c.email) == 0 && strcmp(motDePasse, c.motDePasse) == 0) {
            ...
            effacerConsole();
            afficherLogo();
            afficherCadreCentre(GREEN "Connexion reussie !" RESET);
            _getch();
            return c.id;
        }
    }
...
void modifierProfilClient(int idClient) {
    ...
    while (fscanf(f, "%d %s %s %s %s %s %s\n",
        ...
        if (c.id == idClient) {
            ...
            centrerTexte("Nouveau telephone : "); scanf("%s", c.telephone);
    ...
    afficherCadreCentre(GREEN "Profil modifié avec succes." RESET);
    _getch();
}
